"""
generate.py
Parallelized phase function LUT generator using MieConfig.
"""
import numpy as np
import struct
...
def get_water_ior(wavelength_nm):  # Sellmeier
    ...
    B = [5.084151894e-3, 1.818488474e-2, 2.625439472e-2, 1.073842352e1]  # Daimon & Masumura 2007
    n_squared = 1.0
    for i in range(4):
...
def _process_wavelength(
    ...
    for j, r_um in enumerate(radii):
        x = 2 * np.pi * r_um / w_um
        intensity = backend.intensity_unpolarized(m, x, mu)
        weighted_phase += weights[j] * intensity

    raw_integral = np.sum(weighted_phase) * d_mu * 2 * np.pi
    normalized_phase = weighted_phase / (raw_integral + 1e-12)

    print(f"  [Core:{backend.name}] Finished {w_nm:.1f}nm | Raw Int: {raw_integral:.2f}")
    return index, normalized_phase.astype(np.float32)


def generate_phase_table(config: MieConfig, backend: ScatteringBackend):
    """
    Backend-agnostic generator. Wrapper should choose the backend.
    """
    ...
    results = Parallel(n_jobs=-1)(
        delayed(_process_wavelength)(w, i, radii, weights, mu, d_mu, backend)
...
def generate_mie_table(config: MieConfig = MieConfig()):
    """
    Backwards-compatible entry point: uses miepython backend.
    """
...
def save_binary_file(filename, phase_table, config: MieConfig):
    ...
    # Leave as-is for now (even if deprecated) to avoid breaking any readers/tests
