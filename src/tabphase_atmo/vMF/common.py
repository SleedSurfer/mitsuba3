...
class PerformanceData:
    ...
    def append(self, time: float = 0, spp: int = 0, cumm_spp: int = 0, iteration: int = 0, variance: float = 0,
               mse: float = 0) -> None:
    ...
    def saveToFile(self, fileName: str) -> None:
        with open(fileName, 'w', newline='') as file:
            ...
            for i in range(len(self.time)):
                writer.writerow(
                    [self.time[i], self.spp[i], self.cumm_spp[i], self.iteration[i], self.variance[i], self.mse[i]])
