...
class vMF_PathGuidingIntegrator(mi.SamplingIntegrator):
    ...
    def __init__(self: mi.SamplingIntegrator, props: mi.Properties) -> None:
        ...
        """
        self.sdTree_prev = vMF_KDTree()
        self.sdTree_current = vMF_KDTree()
    ...
    def _get_product_mixture(self, eval_p, ray_d, active_mask, is_volume):
        """Generates the true analytic product mixture Vf ⊗ VL for volumes, and raw VL for surfaces."""
        local_mixture = self.sdTree_prev.query_mixture(eval_p, active_mask)
        w0, w1, w2 = local_mixture.weights
        mu0, mu1, mu2 = local_mixture.mus
        k0, k1, k2 = local_mixture.kappas

        mu_f = ray_d
        k_f = self.phase_kappa

        def vmf_product_lobe(w_i, mu_i, k_i, mu_f, k_f):
            unnorm_mu = mu_i * k_i + mu_f * k_f
            k_k = dr.norm(unnorm_mu)
            mu_k = dr.select(k_k > 1e-6, unnorm_mu / k_k, mu_i)

            safe_k_i = dr.maximum(k_i, 1e-4)
            safe_k_f = dr.maximum(k_f, 1e-4)
            safe_k_k = dr.maximum(k_k, 1e-4)

            term_i = 1.0 - dr.exp(-2.0 * safe_k_i)
            term_f = 1.0 - dr.exp(-2.0 * safe_k_f)
            term_k = 1.0 - dr.exp(-2.0 * safe_k_k)

            frac = (safe_k_i * safe_k_f * term_k) / (dr.two_pi * safe_k_k * term_i * term_f)
            exp_arg = dr.minimum(k_i * (dr.dot(mu_i, mu_k) - 1.0) + k_f * (dr.dot(mu_f, mu_k) - 1.0), 80.0)

            s_k = frac * dr.exp(exp_arg)
            return w_i * s_k, mu_k, k_k

        pw0, pmu0, pk0 = vmf_product_lobe(w0, mu0, k0, mu_f, k_f)
        pw1, pmu1, pk1 = vmf_product_lobe(w1, mu1, k1, mu_f, k_f)
        pw2, pmu2, pk2 = vmf_product_lobe(w2, mu2, k2, mu_f, k_f)

        sum_pw = pw0 + pw1 + pw2
        valid_pw = sum_pw > 1e-6
        pw0 = dr.select(valid_pw, pw0 / sum_pw, 1.0 / 3.0)
        pw1 = dr.select(valid_pw, pw1 / sum_pw, 1.0 / 3.0)
        pw2 = dr.select(valid_pw, pw2 / sum_pw, 1.0 / 3.0)

        # THE FIX: Switch tracks based on physics!
        # If hitting a solid surface, abandon the forward-scattering product and use the raw spatial tree.
        final_w0 = dr.select(is_volume, pw0, w0)
        final_w1 = dr.select(is_volume, pw1, w1)
        final_w2 = dr.select(is_volume, pw2, w2)

        final_mu0 = dr.select(is_volume, pmu0, mu0)
        final_mu1 = dr.select(is_volume, pmu1, mu1)
        final_mu2 = dr.select(is_volume, pmu2, mu2)

        final_k0 = dr.select(is_volume, pk0, k0)
        final_k1 = dr.select(is_volume, pk1, k1)
        final_k2 = dr.select(is_volume, pk2, k2)

        from vMF.Models.vMF_mixture import vMFMixture
        return vMFMixture(
            weights=[final_w0, final_w1, final_w2],
            mus=[final_mu0, final_mu1, final_mu2],
            kappas=[final_k0, final_k1, final_k2]
        )
    ...
    def sample(self: mi.SamplingIntegrator,
        ...
        def loop_body(sampler, ray, depth, L, throughput_weight, ior, active, prev_si, prev_bsdf_pdf, prev_bsdf_delta,
            ...
            if phase is not None:
                phase_value_em, phase_pdf_em = phase.eval_pdf(phase_ctx, mei, ds.d,
            ...
            # THE FIX: Evaluate NEE against the TRUE product mixture!
            product_mix_em = self._get_product_mixture(eval_p, ray.d, active_sdtree_and_em, act_medium_scatter)
            sdtree_pdf_em = product_mix_em.pdf(ds.d)

            surface_pdf_em = self.bsdfSamplingFractionCurrent * scatter_pdf_em + (
                    1.0 - self.bsdfSamplingFractionCurrent) * sdtree_pdf_em
            ...
            if phase is not None:
                # RAW WORLD SPACE: The phase function generates a world vector
                ...
                # THE HYBRID MERGE: Phase stays raw, BSDF still needs si.to_world()!
            ...
            active_vmf_query = do_sdtree_mis
            product_mixture = self._get_product_mixture(eval_p, ray.d, active_vmf_query, act_medium_scatter)

            vmf_samples = mi.Point3f(
                sampler.next_1d(active_sample_sdtree_mis),
                sampler.next_1d(active_sample_sdtree_mis),
                sampler.next_1d(active_sample_sdtree_mis)
            )
            sdtree_dir, sdtree_pdf_sampled = product_mixture.sample(vmf_samples)
            wo_world = dr.select(active_sample_sdtree_mis, sdtree_dir, wo_world)

            # KEEP THIS: The surface BSDF needs local space
            ...
            if phase is not None:
                # RAW WORLD SPACE: The phase function takes sdtree_dir directly
            ...
            sdtree_pdf_eval = product_mixture.pdf(wo_world)
            ...
            raw_mis_weight = dr.select(valid_woPdf, eval_val / woPdf, 0.0)
            mis_weight_final = dr.minimum(raw_mis_weight, 100.0)
            ...
            dr.scatter(self.surfaceInteractionRecord.direction.x, value=wo_world.x, index=globalIndex, active=storeFlag)
            dr.scatter(self.surfaceInteractionRecord.direction.y, value=wo_world.y, index=globalIndex, active=storeFlag)
            dr.scatter(self.surfaceInteractionRecord.direction.z, value=wo_world.z, index=globalIndex, active=storeFlag)
            ...
            # THE FIX: Same here, raw 3D.
            dr.scatter(self.surfaceInteractionRecord.direction_nee.x, value=ds.d.x, index=globalIndex,
                       active=isStoreNEERadiance)
            dr.scatter(self.surfaceInteractionRecord.direction_nee.y, value=ds.d.y, index=globalIndex,
                       active=isStoreNEERadiance)
            dr.scatter(self.surfaceInteractionRecord.direction_nee.z, value=ds.d.z, index=globalIndex,
                       active=isStoreNEERadiance)
    ...
    def perform_global_vmf_update(self):
        record = self.surfaceInteractionRecord
        num_records = dr.width(record.radiance)
        if num_records == 0:
            return

        active = dr.full(mi.Bool, True, num_records)
        leaf_indices = self.sdTree_current.getLeafNodeIndex(record.position, active)

        scalar_radiance = record.radiance
        # THE FIX: Don't throw away the NEE data!
        scalar_radiance_nee = dr.mean(record.radiance_nee)

        # The Dispatch: Pass BOTH normal rays and NEE rays!
        self.sdTree_current.fit_mixtures_to_record(
            leaf_indices=leaf_indices,
            directions=record.direction,
            weights=scalar_radiance,
            directions_nee=record.direction_nee,
            weights_nee=scalar_radiance_nee,
            active=active,
    ...
    def scatterDataIntoSDTree(self: mi.SamplingIntegrator) -> None:
        ...
        # THE FIX: We already compressed the arrays. Everything left is valid.
        # Pass mi.Bool(True) instead of the UInt32 activeIndex.
        valid_mask = mi.Bool(True)
        leaf_indices = self.sdTree_current.getLeafNodeIndex(self.surfaceInteractionRecord.position, valid_mask)

        # Increment the vertCount for those leaves so the tree knows when to spatially split
        dr.scatter_reduce(dr.ReduceOp.Add, self.sdTree_current.kdTreeNode.vertCount, mi.Float(1.0), leaf_indices,
                          valid_mask)
    ...
    def refineAndPrepareSDTreeForNextIteration(self) -> None:
        # 1. Split the spatial KDTree
        self.sdTree_current.setRefinementThreshold( self.iteration )
        self.refine()

        # 2. Run the Global EM Trainer on the newly split tree
        self.perform_global_vmf_update()

        # 3. COPY AFTER TRAINING: Give the trained tree to the integrator for the next pass
        self.sdTree_prev.copyFrom( self.sdTree_current )

        # 4. Reset the vertCount for the next data collection pass
