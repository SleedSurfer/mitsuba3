...
class OblateSpheroidParticle(SphericalParticle):
    def __init__(self, radius_mm: float, aspect_ratio: float = 0.98):#0.98):
    ...
    def intersect(self, rays: PhasorRay) -> Tuple[Bool, Float]:
        ...
        sign_b = dr.select(b >= 0.0, Float(1.0), Float(-1.0))
        ...
        # Order them to t1, t2
        t1 = dr.minimum(root1, root2)
        t2 = dr.maximum(root1, root2)
