...
class DrJitRaytracerBackend(ScatteringBackend):
    ...
    def __init__(self, grid_res: int = 600, num_batches: int = 25, num_phi_bins: int = 1,
        ...
        self.air_turbulence_factor = air_turbulence_factor
    ...
    def intensity_unpolarized(self, m: complex, wavelength_nm: float, theta: np.ndarray,
        ...
        if isinstance(particle, PrismParticle):
            collector = PhotonCollectionSphere(theta_bins=theta_requested, num_phi_bins=self.num_phi_bins)

            return run_photon_pipeline(
                particle, collector, theta_requested, theta_requested,
            ...
            return run_phasor_pipeline(
                self, particle, collector, theta_requested, theta_requested,
