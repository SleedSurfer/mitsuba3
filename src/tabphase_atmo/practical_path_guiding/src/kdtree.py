...
from practical_path_guiding.src.quadtree import QuadTree
...
class KDTreeNode:
	...
	def getBBox(self, idx: mi.UInt32) -> mi.BoundingBox3f:
		"""
            Get mitsuba.BoundingBox3f for the given node index (Decomposed)
        """
...
class KDTree:
	...
	def addDataPropagate(self, surfaceInteractionRecord: SurfaceInteractionRecord) -> None:
		...
		# After traversing the tree, we now know which leaf node each surfaceInteractionRecord belongs to.
	...
	def getLeafNodeIndex(self, position: mi.Vector3f, active: mi.Bool = True) -> mi.UInt32:
		...
		# 4. Execute
		search_active, nodeIndex = dr.while_loop(
			state=loop_state,
			cond=loop_cond,
			body=loop_body
		)
	...
	def saveOBJ( self, fileName: str ) -> None:
		...
		# Get bounding box data (Decomposed for modern Dr.Jit SoA layout)
		min_x, min_y, min_z = self.kdTreeNode.bbox.min.x.numpy(), self.kdTreeNode.bbox.min.y.numpy(), self.kdTreeNode.bbox.min.z.numpy()
		max_x, max_y, max_z = self.kdTreeNode.bbox.max.x.numpy(), self.kdTreeNode.bbox.max.y.numpy(), self.kdTreeNode.bbox.max.z.numpy()
		...
		with open( fileName, 'w' ) as file:
			...
			for i in range( numBoxes ):
				bbox_min = [min_x[i], min_y[i], min_z[i]]
				bbox_max = [max_x[i], max_y[i], max_z[i]]
