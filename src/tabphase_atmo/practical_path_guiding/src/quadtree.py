...
class QuadTreeNode:
	...
	def split(self, idx: mi.UInt32) -> None:
		...
		#         Bounding Box splitting (Decomposed)
		min_x = dr.gather(mi.Float, self.bbox.min.x, idx)
		min_y = dr.gather(mi.Float, self.bbox.min.y, idx)
		bbox_min = mi.Vector2f(min_x, min_y)
		...
		dr.scatter( self.bbox.min.x, bbox_q1.min.x, child_1_index )
		dr.scatter( self.bbox.min.y, bbox_q1.min.y, child_1_index )
		dr.scatter( self.bbox.max.x, bbox_q1.max.x, child_1_index )
		dr.scatter( self.bbox.max.y, bbox_q1.max.y, child_1_index )

		dr.scatter( self.bbox.min.x, bbox_q2.min.x, child_2_index )
		dr.scatter( self.bbox.min.y, bbox_q2.min.y, child_2_index )
		dr.scatter( self.bbox.max.x, bbox_q2.max.x, child_2_index )
		dr.scatter( self.bbox.max.y, bbox_q2.max.y, child_2_index )

		dr.scatter( self.bbox.min.x, bbox_q3.min.x, child_3_index )
		dr.scatter( self.bbox.min.y, bbox_q3.min.y, child_3_index )
		dr.scatter( self.bbox.max.x, bbox_q3.max.x, child_3_index )
		dr.scatter( self.bbox.max.y, bbox_q3.max.y, child_3_index )

		dr.scatter( self.bbox.min.x, bbox_q4.min.x, child_4_index )
		dr.scatter( self.bbox.min.y, bbox_q4.min.y, child_4_index )
		dr.scatter( self.bbox.max.x, bbox_q4.max.x, child_4_index )
		dr.scatter( self.bbox.max.y, bbox_q4.max.y, child_4_index )
	...
	def createRootNode(self, numRootNode: int) -> mi.UInt32:
		...
		dr.scatter( self.bbox.min.x, bbox_min.x, newRootNodeIndex )
		dr.scatter( self.bbox.min.y, bbox_min.y, newRootNodeIndex )
		dr.scatter( self.bbox.max.x, bbox_max.x, newRootNodeIndex )
		dr.scatter( self.bbox.max.y, bbox_max.y, newRootNodeIndex )
...
class QuadTree:
	...
	def copyTree(self, rootIndex: mi.UInt32) -> QuadTreeNode:
		...
		while active:
			...
			if dr.width( parentIndex ) > 0:
				...
				active_parent_1 = parentChildIndex == 1
				active_parent_2 = parentChildIndex == 2
				active_parent_3 = parentChildIndex == 3
				active_parent_4 = parentChildIndex == 4
			...
			# 	Bounding box
			# --- BBOX MIN ---
			bbox_min_x = dr.gather(mi.Float, self.quadTreeNode.bbox.min.x, nodeIndex)
			bbox_min_y = dr.gather(mi.Float, self.quadTreeNode.bbox.min.y, nodeIndex)
			dr.scatter(quadTreeNode.bbox.min.x, bbox_min_x, newNodeIndex)
			dr.scatter(quadTreeNode.bbox.min.y, bbox_min_y, newNodeIndex)

			# --- BBOX MAX ---
			bbox_max_x = dr.gather(mi.Float, self.quadTreeNode.bbox.max.x, nodeIndex)
			bbox_max_y = dr.gather(mi.Float, self.quadTreeNode.bbox.max.y, nodeIndex)
			dr.scatter(quadTreeNode.bbox.max.x, bbox_max_x, newNodeIndex)
			dr.scatter(quadTreeNode.bbox.max.y, bbox_max_y, newNodeIndex)
	...
	def appendQuadTreeNode(self, quadTreeNode: QuadTreeNode) -> mi.UInt32:
		...
		dr.scatter( self.quadTreeNode.bbox.min.x, quadTreeNode.bbox.min.x, scatterValueIndex )
		dr.scatter( self.quadTreeNode.bbox.min.y, quadTreeNode.bbox.min.y, scatterValueIndex )
		dr.scatter( self.quadTreeNode.bbox.max.x, quadTreeNode.bbox.max.x, scatterValueIndex )
		dr.scatter( self.quadTreeNode.bbox.max.y, quadTreeNode.bbox.max.y, scatterValueIndex )
	...
	def sampleQuadTree( self, rootIndex: mi.UInt32, sampler: mi.Sampler, active_sample: mi.Bool = True ) -> mi.Vector3f:
		...
		loop_state = (nodeIndex, active_sample, samplePosition, sampler)

		def loop_cond(nodeIndex, active_sample, samplePosition, sampler):
			return active_sample

		def loop_body(nodeIndex, active_sample, samplePosition, sampler):
			isLeaf = dr.gather(mi.Bool, self.quadTreeNode.isLeaf, nodeIndex, active_sample)

			#
			# If leaf, uniform random position in node (DECOMPOSED VECTOR GATHER)
			#
			min_x = dr.gather(mi.Float, self.quadTreeNode.bbox.min.x, nodeIndex, active_sample)
			min_y = dr.gather(mi.Float, self.quadTreeNode.bbox.min.y, nodeIndex, active_sample)
			bbox_min = mi.Vector2f(min_x, min_y)

			max_x = dr.gather(mi.Float, self.quadTreeNode.bbox.max.x, nodeIndex, active_sample)
			max_y = dr.gather(mi.Float, self.quadTreeNode.bbox.max.y, nodeIndex, active_sample)
			bbox_max = mi.Vector2f(max_x, max_y)
			...
			return (nodeIndex, active_sample, samplePosition, sampler)

		nodeIndex, active_sample, samplePosition, sampler = dr.while_loop(
			state=loop_state,
			cond=loop_cond,
			body=loop_body
		)
	...
	def pdfQuadTree( self, rootIndex: mi.UInt32, direction: mi.Vector3f, active: mi.Bool = True ) -> mi.Float:
		...
		loop_state = (nodeIndex, pdf, active_pdf)

		def loop_cond(nodeIndex, pdf, active_pdf):
			return active_pdf

		def loop_body(nodeIndex, pdf, active_pdf):
			#
			...
			return (nodeIndex, pdf, active_pdf)

		nodeIndex, pdf, active_pdf = dr.while_loop(
			state=loop_state,
			cond=loop_cond,
			body=loop_body
		)
...
if __name__ == '__main__':
	...
	irradianceArray = quadTreeNode.irradiance.numpy()
	print('Copy tree irradiance node: ', irradianceArray)
	printBoldUnderLine('Copy tree irradiance:', irradiadianceArray[0] )
	printBoldUnderLine('Copy tree size:', quadTreeNode.getWidth())
	...
	printBoldUnderLine('Clean Tree irradiance:', irradiadianceArray[0] )
